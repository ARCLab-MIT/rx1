...
# Arm IDs
RIGHT_ARM_IDs = [i for i in range(11, 18)]
LEFT_ARM_IDs = [i for i in range(21, 28)]
print(RIGHT_ARM_IDs)
print(LEFT_ARM_IDs)
...
while 1:
    ...
    # Read position of all STServos in the right arm
    for id in RIGHT_ARM_IDs:
        sts_present_position, sts_present_speed, sts_comm_result, sts_error = packetHandler.ReadPosSpeed(id)
        if sts_comm_result != COMM_SUCCESS:
            print(packetHandler.getTxRxResult(sts_comm_result))
        else:
            print("[ID:%03d] PresPos:%d PresSpd:%d" % (id, sts_present_position, sts_present_speed))
        if sts_error != 0:
            print(packetHandler.getRxPacketError(sts_error))

    # Read position of all STServos in the left arm
    for id in LEFT_ARM_IDs:
        sts_present_position, sts_present_speed, sts_comm_result, sts_error = packetHandler.ReadPosSpeed(id)
        if sts_comm_result != COMM_SUCCESS:
            print(packetHandler.getTxRxResult(sts_comm_result))
        else:
